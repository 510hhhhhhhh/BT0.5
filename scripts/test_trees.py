...
class GetClose(Task):
    ...
    def run(self):
        ...
        if self.blackboard.getbuff == False:
            return TaskStatus.FAILURE
        if ((pow(self.blackboard.MyPose['x'] - goal_x, 2) + pow(self.blackboard.MyPose['y'] - goal_y, 2)) > 0.3):
            if env.sended == False:
                print 'sending center_goal successful'
                if env.isActionAvaliable(goal_x, goal_y, goal_yaw):
                    ...
                    self.controller.send_goal(env.navgoal)  # 发送目标
                    env.sended = True
            print "On the way to get close, distance is %s" % np.sqrt(pow(self.blackboard.MyPose['x'] - goal_x, 2) + pow(self.blackboard.MyPose['y'] - goal_y, 2))
            return TaskStatus.RUNNING
        else:
            print "get closed"
            env.sended = False
...
class TurnAround(Task):
    ...
    def run(self):
        ...
        print 'count %s' % (count)
        if np.abs(self.blackboard.MyPose['theta'] - self.blackboard.lastgoal[2])> 10:
            if env.sended == False:
